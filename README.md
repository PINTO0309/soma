...
- **Measured performance** (CrowdTrack test footage, RTX 3070, WebGPU, default dedicated worker):
  ...
  **When 4 or more people are tracked simultaneously, the onnxruntime-web runtime outperforms LiteRT** thanks to its batched ReID: one batched WebGPU inference amortizes the per-crop cost (~5.8 vs ~9 ms/crop measured at ~10 people), putting SOMA-R end-to-end ahead (9.9 vs 9.2 fps). LiteRT cannot batch (its runtime pins compiled inputs to batch 1), so its per-crop ReID cost scales linearly with the number of people; with only 1-3 people the two runtimes are comparable.
- SOMA-R note: the shipped PersonViT `.tflite` exports are graph-optimized for full WebGPU delegation — the original exports carried rank-5 GATHER/RESHAPE in every attention block, which the GPU delegate rejects ("Tensor dimensions must be less than 5"), silently pushing the whole transformer onto the CPU (~130 ms/crop, 13x slower). [tools/optimize_personvit_tflite.py](tools/optimize_personvit_tflite.py) rewrites the qkv unpack to rank-4 Split-style gathers (bit-identical outputs, batch-1 only). The onnxruntime-web runtime handles the original rank-5 `.onnx` exports natively. OSNet remains impractical on the current LiteRT WebGPU (~1.4 s/crop, slow depthwise convolutions) — prefer the PersonViT embedder in the web runtime.
